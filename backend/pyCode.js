const pyCode = (date, time, data) => `
    ...
    # ------------------------------------------------------------------------------------------------
    first_button = data["first_button"] # Первая кнопка (27-я строка)
    start_message = data["start_message"] # Сообщение-приветсвтие, после /start от юзера (29-я строка)
    tk = data["token"] # Токен (10-я строка)
    # ------------------------------------------------------------------------------------------------
    TOKEN = f'{tk}'
    ...
    def start(message):
        ...
        item1 = types.KeyboardButton(f'{first_button}')
        markup.add(item1)
        bot.send_message(message.chat.id, (f"{start_message}").format(
